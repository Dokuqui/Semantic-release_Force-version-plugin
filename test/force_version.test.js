import { analyzeCommits } from '../src/force_version.js';
...
describe('analyzeCommits function', () => {
    ...
    test('throws an error for invalid FORCE_TYPE_VERSION', async () => {
      ...
      await expect(analyzeCommits(pluginConfig, context)).rejects.toThrowError(
        'Invalid FORCE_TYPE_VERSION'
      );
    });
  
    test('returns FORCE_TYPE_VERSION when it is valid', async () => {
      const validType = 'major';
      const pluginConfig = {};
